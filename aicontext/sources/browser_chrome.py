...
class BrowserChromeSource(DataSource):
    ...
    def get_reference_doc(self) -> str:
        return """# Chrome Browser Reference

Local Chrome browser history (visits and downloads).

## Services
| Service | Description |
|---------|-------------|
| chrome | Local Chrome browser history |
...
FROM activity WHERE source='chrome' AND action='visited'
